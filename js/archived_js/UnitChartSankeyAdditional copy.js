export default function UnitChartSankeyAdditional(
  ...
  const ts = canvas.transition().duration(300);
  ...
  link
    .on("mouseover", function (e, d) {
      ...
      g.selectAll("rect").transition().attr("fill", "lightgray");
      g.select(`#${overedRectId}`).transition().attr("fill", "black");
    })
    .on("mouseout", function (e, d) {
      d3.select(this)
        .transition()
        .attr("stroke-width", (d) => Math.max(1, d.width));
      g.selectAll("rect").transition().attr("fill", "black");
    });

  linksGroups.on("mouseover", function (e, d) {
    let overedLinkGroup = d3.select(this);
    let overedPath = overedLinkGroup.attr("class").split(" ")[1];
    // console.log(overedPath);
    ...
    for (article of articleInPath) {
      ...
      // let fillStatus = articleRect.attr("fill");
      // console.log(fillStatus);
      articleRect.transition().attr("fill", "gray");
    }
  });
  ...
  node
    .on("mouseover", function (e, d) {
      ...
      for (article of articleInNode) {
        ...
        // let fillStatus = articleRect.attr("fill");
        // console.log(fillStatus);
        articleRect.transition().attr("fill", color(d.name));
      }
    })
    .on("mouseout", function (e, d) {
      g.selectAll("rect").transition().attr("fill", "black");
