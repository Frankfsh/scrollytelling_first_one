export default function UnitChart_adtional2(
  ...
  function updateSankey(data_links) {
    ...
    graph.links.forEach((link) => {
      link.path = link.source + "_" + link.target;
    });
    ...
    // async function linkJoin(gl) {
    const link = gl.selectAll("g").data(links, (d) => d.path);

    link.join(
      (enter) =>
        enter
          .append("g")
          .attr("class", (d) => `linkGroup no.${d.id}`)
          .call((enter) =>
            enter
              .append("path")
              .attr("id", (d) => d.pathid)
